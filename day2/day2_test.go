...
import (
	...
	"github.com/daveseco7/advent-of-code-2025/day2"
...
func TestExe1(t *testing.T) {
	...
	t.Run("puzzle sample", func(_ *testing.T) {
		...
		ans := day2.Exe1(lineArray, startingPoint)
		if ans != expected {
...
func TestExe2(t *testing.T) {
	...
	t.Run("puzzle sample", func(_ *testing.T) {
		...
		ans := day2.Exe2(lineArray, startingPoint)
		if ans != expected {
			t.Errorf("invalid response %d", ans)
		}
	})
	...
	t.Run("puzzle short sample", func(_ *testing.T) {
		...
		for _, tc := range tt {
			ans := day2.Exe2(tc.lines, tc.startingPoint)
